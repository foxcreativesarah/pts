...
class wpmss{
	...
	function plugin_scripts_load(){
		wp_enqueue_script( 'wpmssab', plugins_url( 'js/wpmssab.min.js', __FILE__ ), array('jquery'), $this->settings['general']['timestamp'] + 1410, 1 );
		wp_enqueue_script( 'SmoothScroll', plugins_url( 'js/SmoothScroll.min.js', __FILE__ ), array('wpmssab'), '1.4.10', 1 );
		wp_enqueue_script( 'wpmss', plugins_url( 'js/wpmss.min.js', __FILE__ ), array('SmoothScroll'), $this->settings['general']['timestamp'] + 1410, 1 );
	}
